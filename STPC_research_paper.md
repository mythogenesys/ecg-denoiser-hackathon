...
$$
Because $G(I+\alpha\Delta)^{-1}$ has a frequency response proportional to $\tfrac{i\omega_k}{1+\alpha\lambda_k}$, its magnitude is small at high frequencies: the high-frequency components of $\eta$ are strongly suppressed in $G(I+\alpha\Delta)^{-1}\eta$. Therefore, the variance of the gradient error is much smaller than that of the unregularized case $G\eta$, while the bias term only mildly rounds the pulse peak for moderate $\alpha$.
